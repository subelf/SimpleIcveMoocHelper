...
const setting = {
    ...
    最高延迟响应时间: 5000,//毫秒
    最低延迟响应时间: 3000,//毫秒
    组件等待时间: 150,//毫秒 组件包括视频播放器,JQuery等,视网络,设备性能而定,启动失败则调整
    //当视频因网络情况不佳加载视频失败时触发警报,遇此警报请调整组件等待时间
    激活视频跳过警示: true,
    //0-高清 1-清晰 2-流畅 3-原画
...
GM_registerMenuCommand("📝检查脚本配置", function () {
    ...
    当前组件响应时间:${setting.组件等待时间 % (1000 * 60) / 1000}
...
function mediaHandler() {
    try {
        ...
        if (player.getDuration() === 0) {
            ...
            if (setting.激活视频跳过警示) {
                alert("网络不稳定,视频加载失败,请更改组件等待时间,调整到一个合适你网络环境的值,建议直接 10000 起步")
            }
