...
const setting = {
    ...
    // 可以忽略视频不稳定刷的同学可关闭,当视频因网络情况不佳加载视频失败时触发警报,遇此警报请调整组件等待时间
    激活视频跳过警示: true,
    //0-高清 1-清晰 2-流畅 3-原画
...
GM_registerMenuCommand("📝检查脚本配置", function () {
    ...
    网络不稳防漏刷警告: ${setting.激活视频跳过警示 ? "✅打开" : "❌关闭"}
...
function mediaHandler() {
    try {
        ...
        if (player.getDuration() === 0) {
            ...
            if (setting.激活视频跳过警示 && mediaLong !== 0)
                alert("职教云网络不稳定,视频加载失败,请更改组件等待时间,调整到一个合适你网络环境的值,建议直接 10000 起步")
